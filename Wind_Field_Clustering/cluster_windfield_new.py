...
def cluster_windfield(scannumber,
    ...
    targets = scale(v_clean)
    features = np.vstack((x_clean, y_clean, targets))
    features = features.T
    ...
    if cluster_method=="meanshift":
        bandwidth = cluster.estimate_bandwidth(X_train[:,2].reshape(-1,1), quantile=0.15, n_samples=300)
        ms = cluster.MeanShift(bandwidth = bandwidth,bin_seeding=True)
        ms.fit(X_train[:,2].reshape(-1,1))
        cluster_number= ms.predict(X_test[:,2].reshape(-1,1))
        labels_ms = ms.labels_
        _,label_test,_,data_test = train_test_split(labels_ms,X_train[:,2].reshape(-1,1),test_size= 0.1, random_state=r_state)
    ...
    if cluster_method=="twomeans":
        ...
        two_means.fit(X_train[:,2].reshape(-1,1))
        cluster_number = two_means.predict(X_test[:,2].reshape(-1,1))
    ...
    if cluster_method=="kmeans":
        ...
        kmeans.fit(X_train[:,2].reshape(-1,1))
        cluster_number = kmeans.predict(X_test[:,2].reshape(-1,1))
